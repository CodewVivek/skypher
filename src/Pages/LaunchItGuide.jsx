...
import { Search, ChevronDown, ChevronUp, Info, UserPlus, Rocket, Edit, Link2, Settings, LayoutGrid, MessageCircle, Bell, HelpCircle, Star, AlertTriangle, Shield, Users, Globe, FileText, Image, Trash2, RefreshCw, LogIn, LogOut, AlertCircle } from 'lucide-react';
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 pb-16">
            <div className="max-w-3xl mx-auto px-4 pt-10">
                ...
                <h1 className="text-4xl font-extrabold text-blue-700 mb-6 text-center drop-shadow-sm">LaunchIT Guide & FAQ</h1>
                <p className="text-lg text-gray-600 mb-10 text-center">Your complete resource for using LaunchIT. Search or browse below for answers, tips, and platform walkthroughs.</p>
                {filteredSections.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-24">
                        <HelpCircle className="w-12 h-12 text-blue-300 mb-4 animate-bounce" />
                        <div className="text-xl text-gray-400 font-medium">No results found for your search.</div>
                        <div className="text-sm text-gray-400 mt-2">Try a different keyword or browse the guide below.</div>
                    </div>
                ) : (
                        ...
                        {filteredSections.map((section, idx) => (
                            ...
                            >
                                <div className="flex items-center gap-3 px-6 py-5">
                                    ...
                                    <h2 className="flex-1 text-lg md:text-xl font-semibold text-blue-900">{section.heading}</h2>
                                    <span className="ml-2">
                                        {openIdx === idx ? (
                                ...
                                {openIdx === idx && (
                                    <div className="px-6 pb-6 pt-0">
                                        <pre className="whitespace-pre-wrap text-base text-gray-700 font-sans leading-relaxed mt-2">{section.content}</pre>
                                    </div>
                                )}
                            </div>
                        ))}
