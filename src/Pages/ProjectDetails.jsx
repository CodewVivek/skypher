...
const NextArrow = ({ onClick, style, ...rest }) => (
  <button
    type="button"
    onClick={onClick}
    className="absolute top-1/2 right-4 transform -translate-y-1/2 z-10 bg-white/80 dark:bg-gray-800/80 hover:bg-white dark:hover:bg-gray-800 shadow rounded-full p-2 flex items-center justify-center transition-colors duration-300"
    style={{ ...style }}
    aria-label="Next image"
  >
    <ArrowBigRight className="w-6 h-6 text-gray-700 dark:text-gray-300" />
  </button>
);
...
const PrevArrow = ({ onClick, style, ...rest }) => (
  <button
    type="button"
    onClick={onClick}
    className="absolute top-1/2 left-4 transform -translate-y-1/2 z-10 bg-white/80 dark:bg-gray-800/80 hover:bg-white dark:hover:bg-gray-800 shadow rounded-full p-2 flex items-center justify-center transition-colors duration-300"
    style={{ ...style }}
    aria-label="Previous image"
  >
    <ArrowBigLeft className="w-6 h-6 text-gray-700 dark:text-gray-300" />
  </button>
);
...
const ProjectDetails = () => {
  ...
  const prevModal = () =>
    setModalIndex((i) => (i === 0 ? project.cover_urls.length - 1 : i - 1));
  const nextModal = () =>
    setModalIndex((i) => (i === project.cover_urls.length - 1 ? 0 : i + 1));
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-900 transition-colors duration-300">
      <div className="w-full flex items-center justify-center min-h-[260px] bg-gradient-to-b from-gray-100 to-white dark:from-gray-800 dark:to-gray-900">
        <div className="flex flex-col items-center justify-center w-full max-w-2xl px-4 py-12 text-center m-10">
          {project.logo_url && (
            <img
              src={project.logo_url}
              alt={`${project.name} logo`}
              className="w-16 h-16 rounded-2xl border-4 border-white dark:border-gray-700 shadow-lg object-contain mb-4 mx-auto"
            />
          )}
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-center justify-center">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-full px-3 py-2 shadow-sm hover:shadow-md border border-gray-200 dark:border-gray-700 transition-all duration-300 inline-flex items-center justify-center">
            ...
            <div className="mb-8 relative">
              ...
              {modalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80">
                  ...
                  <button
                    className="absolute left-4 top-1/2 transform -translate-y-1/2 bg-white/80 dark:bg-gray-800/80 hover:bg-white dark:hover:bg-gray-800 shadow rounded-full p-2 z-10 transition-colors duration-300"
                    onClick={prevModal}
                    style={{
                      display: project.cover_urls.length > 1 ? "block" : "none",
                    }}
                    aria-label="Previous image"
                  >
                    <ArrowBigLeft className="w-8 h-8 text-gray-700 dark:text-gray-300" />
                  </button>
                  <img
                    src={project.cover_urls[modalIndex]}
                    alt={`Cover ${modalIndex + 1}`}
                    className="max-h-[80vh] max-w-[90vw] rounded-lg shadow-lg object-contain bg-white dark:bg-gray-800"
                  />
                  <button
                    className="absolute right-4 top-1/2 transform -translate-y-1/2 bg-white/80 dark:bg-gray-800/80 hover:bg-white dark:hover:bg-gray-800 shadow rounded-full p-2 z-10 transition-colors duration-300"
                    onClick={nextModal}
                    style={{
                      display: project.cover_urls.length > 1 ? "block" : "none",
                    }}
                    aria-label="Next image"
                  >
                    <ArrowBigRight className="w-8 h-8 text-gray-700 dark:text-gray-300" />
                  </button>
                </div>
              )}
            </div>
            ...
            (!project.cover_urls || project.cover_urls.length === 0) && (
              <div className="mb-8">
                <img
                  ...
                  alt="Thumbnail"
                  className="rounded-lg border border-gray-200 dark:border-gray-700 w-full max-h-[300px] object-contain bg-white dark:bg-gray-800"
                />
              </div>
            )}
          <ReportModal
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-400 dark:border-gray-600 p-6 space-y-6 transition-all hover:shadow-2xl duration-300">
                ...
                <ul className="space-y-1 text-sm text-gray-700 dark:text-gray-300">
                  {project.links.map((link, idx) => (
                    <li key={idx} className="flex items-start gap-2">
                      <Link className="w-4 h-4 mt-1 text-gray-500 dark:text-gray-400" />
                      <a
                        href={link}
                        target="_blank"
                        rel="noopener noreferrer"
            ...
            <div>
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-red-100 dark:bg-red-900/20 text-red-700 dark:text-red-300 rounded-xl hover:bg-red-200 dark:hover:bg-red-900/30 transition-all duration-200 font-medium text-sm"
              >
